...
if __name__ == "__main__":
    ...
    while train_accu < 0.99:
        ...
        optim.step()
        # Cross Entropy
        max_index_train = torch.max(model_2_layers(X_train_, w0, w1, w2, b0, b1, b2), dim=1).indices.numpy()
        ...
        max_index_test = torch.max(model_2_layers(X_test_, w0, w1, w2, b0, b1, b2), dim=1).indices.numpy()
        ...
        loss_test_list.append(torch.nn.functional.cross_entropy(model_2_layers(X_test_, w0, w1, w2, b0, b1, b2), y_test_))
        print("##################", iter, "##################")
    ...
    plt.legend()
    plt.savefig("/Users/yinruideng/Desktop/senior_spring/cse546/hw/hw3/latex/plots/A5b_2.png")
    plt.show()

    w0.shape[0] * w0.shape[1] + w1.shape[0] * w1.shape[1] + w2.shape[0] * w2.shape[1] + len(b0) + len(b1) + len(b2)


# CROSS ENTROPY: Train Accuracy:  0.99005
# CROSS ENTROPY: Test Accuracy:  0.9624
# Train Loss:  tensor(0.0409, grad_fn=<NllLossBackward>)
# Test Loss:  tensor(0.1531, grad_fn=<NllLossBackward>)
#####################################################################################################################
